...
def atomic_write_json(path: str | Path, payload: dict[str, Any], overwrite: bool = False) -> Path:
    return atomic_write_text(path, json.dumps(json_safe(payload), indent=2, sort_keys=True) + "\n", overwrite=overwrite)
...
def positive_claim_hits(text: str) -> list[str]:
    phrases = [
        "proves fate",
        ...
        "terminal fate",
        "reports fate probability",
        "fate probability computed",
